...
main(int argc, char *argv[])
{
  ...
  1b:	8b 70 04             	mov    0x4(%eax),%esi
  int stringLength = strlen(curr_string);
  1e:	56                   	push   %esi
  1f:	8d 5e 01             	lea    0x1(%esi),%ebx
  char* curr_string = argv[1];
  22:	89 75 dc             	mov    %esi,-0x24(%ebp)
  int stringLength = strlen(curr_string);
  25:	e8 26 01 00 00       	call   150 <strlen>
  char temp;

  for (int i = 0; i < stringLength - 1; i++) {
  2a:	83 c4 10             	add    $0x10,%esp
  2d:	8d 78 ff             	lea    -0x1(%eax),%edi
  int stringLength = strlen(curr_string);
  30:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  for (int i = 0; i < stringLength - 1; i++) {
  33:	01 c6                	add    %eax,%esi
  35:	89 7d e0             	mov    %edi,-0x20(%ebp)
  38:	31 ff                	xor    %edi,%edi
  3a:	83 f8 01             	cmp    $0x1,%eax
  3d:	7e 30                	jle    6f <main+0x6f>
  3f:	90                   	nop
    for (int j = i + 1; j < stringLength; j++) {
  40:	83 c7 01             	add    $0x1,%edi
  43:	89 d8                	mov    %ebx,%eax
  45:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
  48:	7e 1d                	jle    67 <main+0x67>
  4a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      if (curr_string[i] > curr_string[j]) {
  50:	0f b6 53 ff          	movzbl -0x1(%ebx),%edx
  54:	0f b6 08             	movzbl (%eax),%ecx
  57:	38 ca                	cmp    %cl,%dl
  59:	7e 05                	jle    60 <main+0x60>
        temp = curr_string[i];
        curr_string[i] = curr_string[j];
  5b:	88 4b ff             	mov    %cl,-0x1(%ebx)
        curr_string[j] = temp;
  5e:	88 10                	mov    %dl,(%eax)
    for (int j = i + 1; j < stringLength; j++) {
  60:	83 c0 01             	add    $0x1,%eax
  63:	39 f0                	cmp    %esi,%eax
  65:	75 e9                	jne    50 <main+0x50>
  for (int i = 0; i < stringLength - 1; i++) {
  67:	83 c3 01             	add    $0x1,%ebx
  6a:	3b 7d e0             	cmp    -0x20(%ebp),%edi
  6d:	75 d1                	jne    40 <main+0x40>
      }
    }
  }
  ...
  72:	68 18 08 00 00       	push   $0x818
  77:	e8 07 03 00 00       	call   383 <unlink>
  fd = open("sort_string.txt", O_CREATE | O_RDWR);
  7c:	5a                   	pop    %edx
  7d:	59                   	pop    %ecx
  7e:	68 02 02 00 00       	push   $0x202
  83:	68 18 08 00 00       	push   $0x818
  88:	e8 e6 02 00 00       	call   373 <open>
  if(fd < 0){
  ...
  94:	78 1c                	js     b2 <main+0xb2>
    printf(1, "Something went wrong in opening file\n");
  }
  write(fd, curr_string, stringLength);
  96:	83 ec 04             	sub    $0x4,%esp
  99:	ff 75 e4             	pushl  -0x1c(%ebp)
  9c:	ff 75 dc             	pushl  -0x24(%ebp)
  9f:	53                   	push   %ebx
  a0:	e8 ae 02 00 00       	call   353 <write>
  close(fd);
  a5:	89 1c 24             	mov    %ebx,(%esp)
  a8:	e8 ae 02 00 00       	call   35b <close>
  
  exit();
  ad:	e8 81 02 00 00       	call   333 <exit>
    printf(1, "Something went wrong in opening file\n");
  b2:	50                   	push   %eax
  b3:	50                   	push   %eax
  b4:	68 28 08 00 00       	push   $0x828
  b9:	6a 01                	push   $0x1
  bb:	e8 f0 03 00 00       	call   4b0 <printf>
  c0:	83 c4 10             	add    $0x10,%esp
  c3:	eb d1                	jmp    96 <main+0x96>
  c5:	66 90                	xchg   %ax,%ax
  c7:	66 90                	xchg   %ax,%ax
  c9:	66 90                	xchg   %ax,%ax
  cb:	66 90                	xchg   %ax,%ax
...
printf(int fd, const char *fmt, ...)
{
 ...
 634:	eb 1a                	jmp    650 <printf+0x1a0>
          s = "(null)";
 636:	bb 4e 08 00 00       	mov    $0x84e,%ebx
        while(*s != 0){
